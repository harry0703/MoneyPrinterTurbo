...
class StatsPageScopeTest(unittest.TestCase):
    """
    三个账号的会话在这一页第一次运行之后失效。页面必须只列允许的账号，
    并说清楚其余的为什么不在——否则下次看到只有一个账号会以为是坏了。
    """

    def test_it_lists_only_the_allowed_accounts(self):
        self.assertIn("stats_accounts()", _source())

    def test_it_does_not_enumerate_every_account_for_display(self):
        tree = _tree()
        loader = next(
            node for node in tree.body
            if isinstance(node, ast.FunctionDef) and node.name == "account_labels"
        )
        self.assertNotIn("list_accounts", ast.unparse(loader))

    def test_the_held_back_accounts_are_explained(self):
        source = _source()
        self.assertIn("withheld", source)
        self.assertIn("sessions expired", source)
