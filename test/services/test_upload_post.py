...
class TestUploadPostRequestPayload(unittest.TestCase):
    @patch("app.services.upload_post.config.app", _CONFIG_BASE)
    @patch("app.services.upload_post.os.path.exists", return_value=True)
    @patch("builtins.open", mock_open(read_data=b"fake"))
    @patch("app.services.upload_post.requests.post")
    def test_endpoint_and_platform_format(self, mock_post, _exists):
        mock_post.return_value = _mock_response()

        UploadPostService().upload_video("/fake/v.mp4", "T")

        call_url = mock_post.call_args[0][0]
        self.assertTrue(call_url.endswith("/api/upload"), f"wrong endpoint: {call_url}")

        data = mock_post.call_args[1]["data"]
        platforms = _get_all(data, "platform[]")
        self.assertEqual(platforms, ["tiktok", "instagram"])
        self.assertEqual(_get(data, "user"), "testuser")

    @patch("app.services.upload_post.config.app", _CONFIG_BASE)
    @patch("app.services.upload_post.os.path.exists", return_value=True)
    @patch("builtins.open", mock_open(read_data=b"fake"))
    @patch("app.services.upload_post.requests.post")
    def test_request_never_carries_youtube_fields(self, mock_post, _exists):
        """YouTube 已改由官方接口发布，转发请求不能再带它的专属字段，
        否则同一个成片会被发布两次。"""
        mock_post.return_value = _mock_response()

        UploadPostService().upload_video(
            "/fake/v.mp4", "T", platforms=["tiktok", "youtube"]
        )

        data = mock_post.call_args[1]["data"]
        self.assertFalse(_has_key(data, "youtube_title"))
        self.assertFalse(_has_key(data, "youtube_description"))
        self.assertFalse(_has_key(data, "tags[]"))
