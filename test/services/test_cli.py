...
class TestCli(unittest.TestCase):
    ...
    def test_tensorscale_video_source_requires_explicit_charge_confirmation(self):
        with self.assertRaises(SystemExit) as raised:
            cli.parse_args(
                ["--video-subject", "test", "--video-source", "tensorscale"]
            )
        self.assertEqual(raised.exception.code, 2)
        ...
        self.assertEqual(cli.build_video_params(args).video_source, "tensorscale")

    def test_tensorscale_confirmation_is_not_required_before_material_stage(self):
        args = cli.parse_args(
            [
                "--video-subject",
                "test",
        ...
        self.assertEqual(args.video_source, "tensorscale")
    ...
    def test_batch_tensorscale_source_uses_global_charge_confirmation(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            manifest = Path(temp_dir) / "tasks.json"
            manifest.write_text(
                json.dumps(
                    [
                        {
                            "video_subject": "TensorScale batch task",
                            "video_source": "tensorscale",
                        }
                    ]
                ),
                encoding="utf-8",
            ...
            with (
                ...
                patch("app.utils.utils.get_uuid", return_value="task-tensorscale"),
                redirect_stdout(io.StringIO()),
            ):
                accepted = cli.run_cli(
                    [
