...
def test_webui_upload_post_checkboxes_independent_and_preserve_state():
    # Split state: enabled=True, auto_upload=False
    ...
    with (
        ...
        # Opening Settings dialog should leave split configuration untouched
        assert config.app["upload_post_enabled"] is True
        assert config.app["upload_post_auto_upload"] is False

        enabled_cb = _widget_by_key(app.checkbox, "upload_post_enabled_checkbox")
        auto_cb = _widget_by_key(app.checkbox, "upload_post_auto_upload_checkbox")
        assert enabled_cb.value is True
        assert auto_cb.value is False

        # Toggling auto_upload updates only auto_upload
