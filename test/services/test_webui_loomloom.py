...
def test_loomloom_script_choice_keeps_original_single_generation_flow():
    test_config = dict(
        config.app,
        llm_provider="openai",
    ...
    with (
        ...
        patch.object(loomloom.LoomLoomScriptBackend, "quote") as quote_call,
        patch.object(loomloom.LoomLoomScriptBackend, "execute") as execute_call,
        patch.object(llm, "generate_script", return_value="generated script") as script_call,
        patch.object(llm, "generate_terms", return_value=["term-a", "term-b"]),
    ):
        ...
        assert _widget_by_key(app.button, "auto_generate_script")
        assert _widget_by_key(app.button, "auto_generate_terms")
        assert all(item.key != "loomloom_quote_scripts" for item in app.button)
        assert all(item.key != "loomloom_execute_scripts" for item in app.button)
        assert all(item.key != "loomloom_candidate_count" for item in app.number_input)
        ...
        assert script_call.call_count == 1
        assert app.session_state["video_script"] == "generated script"
        assert app.session_state["video_terms"] == "term-a, term-b"
        assert quote_call.call_count == 0
...
def test_generated_long_script_autofills_video_count_once_and_shows_shortfall():
    ...
    with (
        ...
        assert "black screen" in warning_text
        assert "theoretically cover at most 15.0 sec" in warning_text
...
def test_loomloom_video_source_quotes_then_passes_secret_in_confirmed_request():
    ...
    with (
        ...
        assert "https://console.shengsuanyun.com/user/keys" in _widget_by_key(
            app.text_input, "loomloom_user_api_token"
        ).help
...
def test_selected_shengsuanyun_provider_hides_duplicate_loomloom_key_input():
    ...
    with (
        ...
        assert _widget_by_key(app.button, "auto_generate_terms")
        assert all(item.key != "loomloom_quote_scripts" for item in app.button)
        assert all("已复用" not in str(item.value) for item in app.caption)
        assert all(
...
def test_paused_legacy_script_run_does_not_replace_original_generation_ui():
    test_config = dict(
        config.app,
        llm_provider="openai",
        script_generation_backend="loomloom",
    ...
    with (
        ...
        assert _widget_by_key(app.button, "auto_generate_script")
        assert all(item.key != "loomloom_quote_scripts" for item in app.button)
        assert all(item.key != "loomloom_resume_status_check" for item in app.button)
        assert all(item.key != "loomloom_stop_tracking_run" for item in app.button)
        assert [str(item.value) for item in app.exception] == []
