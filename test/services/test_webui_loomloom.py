...
def test_loomloom_video_source_quotes_and_passes_secret_only_in_runtime_context():
    test_config = dict(
        config.app,
        script_generation_backend="loomloom",
        loomloom_base_url="https://example.test/loom/v1",
    ...
    quote_result = loomloom.LoomLoomQuote(
        ...
        estimated_buyer_payable_amount="0.1230000",
        input_rows=(),
    )
    ...
    with (
        ...
        runtime_context = submit_generation.call_args.kwargs["runtime_context"]
        assert "session-user-token" not in submitted_params.model_dump_json()
        assert (
            runtime_context["loomloom_video_settings"].api_token
            == "session-user-token"
        )
        assert "session-user-token" not in repr(
            runtime_context["loomloom_video_settings"]
        )
        assert runtime_context["loomloom_video_listing_version_id"] == "video-version"
        assert [str(item.value) for item in app.exception] == []
