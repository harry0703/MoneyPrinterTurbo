...
class TestLiteLLMProvider(unittest.TestCase):
    ...
    def _patch_dashscope_multimodal(self, response):
        def to_obj(value):
            if isinstance(value, dict):
                converted = {}
                for key, item in value.items():
                    # dashscope MultiModalConversation 的 content 是
                    # [{"text": ...}] 形式，保持 dict 元素原样，其余包装层
                    # 转成属性访问对象以匹配真实响应结构。
                    converted[key] = item if key == "content" else to_obj(item)
                return types.SimpleNamespace(**converted)
            if isinstance(value, list):
                return [to_obj(item) for item in value]
            return value

        fake_response = to_obj(response)
        ...
        fake_dashscope = types.SimpleNamespace(
            ...
            MultiModalConversation=types.SimpleNamespace(
                call=lambda **kwargs: fake_response
            ),
        )
    ...
    def test_qwen_provider_reads_chat_choices_content(self):
        """
        DashScope MultiModalConversation 会把文本放在
        `output.choices[0].message.content[0]["text"]`。这里覆盖该响应形态，
        避免 qwen3 系列模型（关闭深度思考）返回的结构无法取值。
        """
        ...
        with self._patch_dashscope_multimodal(response):
            result = llm._generate_response("Say hello")
