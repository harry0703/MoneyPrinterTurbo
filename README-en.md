...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=MoneyPrinterTurbo)** is a full-modal, OpenAI-compatible AI inference platform — plug it in as a drop-in backend for MoneyPrinterTurbo's script generation to access DeepSeek, Qwen, GLM, Kimi, MiniMax and more through a single API, no multi-vendor setup needed. Budget-friendly [coding plan](https://www.atlascloud.ai/console/coding-plan).

Pick `Atlas Cloud` as the `LLM Provider` in the WebUI, or configure it in `config.toml`:
...
`deepseek-ai/deepseek-v4-pro` is a reasoning model — if you pass `max_tokens`, give it `>= 512`.
