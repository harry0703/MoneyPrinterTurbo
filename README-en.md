...
`ARK_API_KEY` and `VOLCENGINE_ARK_API_KEY` are supported when the config value is empty. Select **Volcano Engine Seedance** as the video source and explicitly confirm paid generation before starting. CLI users must add `--confirm-seedance-charge`.
