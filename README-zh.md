...
简体中文 | [English](README.md) | [版本发布](https://github.com/harry0703/MoneyPrinterTurbo/releases) | [问题反馈](https://github.com/harry0703/MoneyPrinterTurbo/issues)
...
克隆仓库后，只需一条命令即可完成安装并启动。首次运行会自动安装 uv、Python 3.11 和全部依赖：

- Windows：双击或在终端执行 `webui.bat`
- macOS / Linux：执行 `sh webui.sh`

也可以先单独执行安装脚本：`install.bat`（Windows）或 `sh install.sh`（macOS / Linux）。
