...
const parseVideoScript = async () => {
  ...
  try {
    ...
    if (response.status === 'success' || response.status === 'manual') {
      ...
      // Always update video title from backend (which generates attractive and surprising titles)
      // regardless of whether user has already entered a title
      if (response.video_title && response.video_title.length > 0) {
        let generatedTitle = response.video_title.trim();
        // Truncate to 16 characters to keep it concise for the video opening
        if (generatedTitle.length > 16) {
          generatedTitle = generatedTitle.substring(0, 16);
        }
        scriptStore.updateVideoTitle(generatedTitle);
        console.info(`Auto-generated video title from backend: '${generatedTitle}'`);
      }
