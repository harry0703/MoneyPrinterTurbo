...
Task 8 的独立边界脚本只在完全合成的临时根上演练：它内置受审 BASE、精确 240 删除集摘要和 MediaCrawler commit，并实际核对 Git Raw/依赖/配置边界、路径链、敏感 JSON 键与媒体文件头。任一项缺失或不可验都非零失败，不会用 skip 代替通过。这一合成验证不声称现实热度、医学有效性或真实人工审批。
