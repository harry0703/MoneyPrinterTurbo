...
const episodes = [
  ...
  {id:'002', topic:'吃饭快慢与下午感受', cover:'吃饭快慢这样观察', src:['S04','S03'], cards:[
  ...
  {id:'003', topic:'午餐太撑后怎样安排', cover:'吃撑以后先把节奏放稳', src:['S01','S04'], cards:[
  ...
  {id:'004', topic:'饭后坐着或轻走的感受', cover:'饭后坐走感受这样比', src:['S03','S06'], cards:[
  ...
  {id:'005', topic:'下午没精神的小重启', cover:'下午卡住这样小重启', src:['S03'], cards:[
  ...
  {id:'006', topic:'午休后更困先看时点', cover:'午休醒后先看三个时点', src:['S10','S07'], cards:[
  ...
  {id:'009', topic:'下午难任务何时做更顺', cover:'难任务换时点再比较', src:[], cards:[
...
function icon(kind,x,y,scale=1,fg=C.teal,bg=C.peach){
  ...
  const shapes={
    ...
    bowl:`<path d="M${X(22)} ${Y(68)}L${X(140)} ${Y(68)}Q${X(132)} ${Y(146)} ${X(81)} ${Y(146)}Q${X(30)} ${Y(146)} ${X(22)} ${Y(68)}Z" fill="${bg}" stroke="${fg}" stroke-width="${S(9)}"/><path d="M${X(44)} ${Y(42)}Q${X(56)} ${Y(18)} ${X(68)} ${Y(42)}M${X(82)} ${Y(42)}Q${X(94)} ${Y(18)} ${X(106)} ${Y(42)}" stroke="${fg}" stroke-width="${S(7)}" fill="none" stroke-linecap="round"/>`,
    ...
    window:`<rect x="${X(18)}" y="${Y(50)}" width="${S(36)}" height="${S(58)}" rx="${S(10)}" fill="${bg}"/><rect x="${X(66)}" y="${Y(34)}" width="${S(36)}" height="${S(74)}" rx="${S(10)}" fill="${C.fresh}"/><rect x="${X(114)}" y="${Y(50)}" width="${S(36)}" height="${S(58)}" rx="${S(10)}" fill="${bg}"/><circle cx="${X(84)}" cy="${Y(132)}" r="${S(12)}" fill="${fg}"/>`,
    ...
    cups:`<circle cx="${X(50)}" cy="${Y(84)}" r="${S(34)}" fill="${bg}"/><circle cx="${X(112)}" cy="${Y(84)}" r="${S(34)}" fill="${C.fresh}"/><path d="M${X(50)} ${Y(38)}v92M${X(112)} ${Y(38)}v92" stroke="${fg}" stroke-width="${S(8)}"/>`,
...
function makeCard(ep,index){
  ...
  return base(`${marker('07')}<circle cx="486" cy="520" r="230" fill="${C.peach}"/><path d="M286 550c48-210 310-250 420-70" fill="none" stroke="${C.teal}" stroke-width="34" stroke-linecap="round"/><circle cx="710" cy="492" r="28" fill="${C.fresh}"/>${t(486,532,'留作下次',48,700,C.ink,'middle')}<rect x="72" y="790" width="828" height="454" rx="24" fill="${C.teal}"/>${multiline(120,910,wrap(title,10),70,88,700,C.cream)}${multiline(120,1120,wrap(copy,16),42,68,400,C.cream)}<rect x="72" y="1292" width="828" height="120" rx="24" fill="${C.peach}"/>${t(486,1368,'不打分 · 不强求 · 安全优先',40,700,C.ink,'middle')}${footer()}`);
}
...
function makeCover(ep){
  ...
  if(v===1){
    const scene=has?image(primary,72,700,828,600,`cover${ep.id}`,'xMidYMid'):abstractScene(72,700,828,600,v);
...
function runMagick(args,cwd=repo){
  const r=spawnSync('magick',args,{cwd,encoding:'utf8'});
