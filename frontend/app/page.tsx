...
function GenerationDialog({ task, onMinimize, onClose }: { task: Task; onMinimize: () => void; onClose: () => void }) {
  ...
  const activeStage = task.current_stage || events[events.length - 1]?.stage || "render";
  const stageEmoji: Record<string, string> = { queue: "🛰️", preflight: "🧭", script: "✍️", terms: "🧠", audio: "🎙️", subtitle: "💬", materials: "🎞️", render: "🎬", complete: "🎉" };
  const activeEmoji = isFailed ? "🧯" : isComplete ? "🎉" : stageEmoji[activeStage] || "✨";
  return <aside className="generation-popup fixed bottom-5 right-5 z-40 flex max-h-[min(720px,calc(100vh-2rem))] w-[min(460px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-white/15 bg-[#15131b]/[.98] shadow-2xl shadow-black/50 backdrop-blur-xl" role="dialog" aria-label="Video generation progress">
    ...
    {!isComplete && <div className="shrink-0 px-5 pt-4"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-semibold text-white">{currentLabel}</p><p className="mt-1 text-xs text-[#858b96]">The Python engine is working in the background.</p></div><span className="text-lg font-bold text-[#ff9b9f]">{task.progress || 0}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/[.08]"><div className="h-full rounded-full bg-gradient-to-r from-[#ff5b62] via-[#ff8f94] to-[#b973ff] transition-all duration-700" style={{ width: `${task.progress || 0}%` }} /></div></div>}
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">{isComplete && task.videos?.[0] && <div className="mb-2 overflow-hidden rounded-xl border border-white/10 bg-black"><div className="flex items-center justify-between border-b border-white/10 bg-white/[.03] px-3 py-2"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#858b96]">Finished video</p><a className="text-[11px] font-semibold text-[#ff9b9f]" href={assetUrl(task.videos[0])} target="_blank" rel="noreferrer">Open file ↗</a></div><video className="max-h-[26rem] w-full bg-black object-contain" controls playsInline src={assetUrl(task.videos[0])} /></div>}{!isComplete && <><div className="generation-feed-intro mb-4 flex items-start gap-3"><span className="generation-agent-avatar grid h-9 w-9 shrink-0 place-items-center rounded-xl text-lg">{activeEmoji}</span><div><p className="text-sm font-semibold text-white">{isFailed ? "The engine needs a hand" : "I’m building your short"}</p><p className="mt-1 text-xs leading-5 text-[#858b96]">{isFailed ? currentLabel : "I’ll work through the story, visuals, voice, and timing for you."}</p></div></div><div className="generation-constellation relative mx-1 min-h-64">{latestEvents.map((event, index) => <div key={event.id || `${event.stage}-${index}`} className={`generation-node generation-node-${index} absolute flex gap-2 rounded-xl border p-2.5 ${index === 0 && isProcessing ? "generation-node-active" : "border-white/10 bg-white/[.03]"}`}><span className={`generation-feed-emoji relative z-10 grid h-6 w-6 shrink-0 place-items-center rounded-lg text-sm ${index === 0 && isProcessing ? "generation-feed-emoji-active" : ""}`}>{event.level === "error" ? "🧯" : stageEmoji[event.stage || ""] || "✨"}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-[#e8e9ed]">{event.message}</p><p className="mt-1 line-clamp-2 text-[10px] leading-4 text-[#737a86]">{event.stage === "script" ? "Shaping the hook and narration beats." : event.stage === "terms" ? "Turning the idea into visual search signals." : event.stage === "audio" ? "Giving the narration a natural voice." : event.stage === "subtitle" ? "Matching captions to the spoken timing." : event.stage === "materials" ? "Finding footage that fits the story." : event.stage === "render" ? "Compositing everything into the final frame." : "The creative engine is getting ready."}</p></div></div>)}{isProcessing && <div className="generation-thinking absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-[#ff5b62]/35 bg-[#ff5b62]/[.1] px-3 py-2"><span className="generation-feed-emoji generation-feed-emoji-active grid h-5 w-5 place-items-center rounded-md text-xs">✨</span><span className="whitespace-nowrap text-[11px] font-semibold text-white">{currentLabel}<span className="generation-typing">...</span></span></div>}</div></>}
    </div>
    <div className="flex shrink-0 items-center justify-between border-t border-white/10 px-5 py-3"><p className="text-[11px] text-[#858b96]">{isComplete ? "Preview your finished short." : "You can keep working while this runs."}</p>{isComplete && task.videos?.[0] && <a className="button button-primary !min-h-8 !px-3" href={assetUrl(task.videos[0])} target="_blank" rel="noreferrer">Open video ↗</a>}</div>
  </aside>;
}
