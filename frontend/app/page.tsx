...
function GenerationDialog({ task, onMinimize, onClose }: { task: Task; onMinimize: () => void; onClose: () => void }) {
  ...
  return <aside className="generation-popup fixed bottom-5 right-5 z-40 flex max-h-[min(720px,calc(100vh-2rem))] w-[min(460px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-white/15 bg-[#15131b]/[.98] shadow-2xl shadow-black/50 backdrop-blur-xl" role="dialog" aria-label="Video generation progress">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">{isComplete && task.videos?.[0] && <div className="mb-5 overflow-hidden rounded-xl border border-white/10 bg-black"><div className="flex items-center justify-between border-b border-white/10 bg-white/[.03] px-3 py-2"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#858b96]">Generated preview</p><a className="text-[11px] font-semibold text-[#ff9b9f]" href={assetUrl(task.videos[0])} target="_blank" rel="noreferrer">Open file ↗</a></div><video className="max-h-72 w-full bg-black object-contain" controls playsInline src={assetUrl(task.videos[0])} /></div>}<div className="generation-feed-intro mb-5 flex items-start gap-3"><span className="generation-agent-avatar grid h-9 w-9 shrink-0 place-items-center rounded-xl text-lg">{activeEmoji}</span><div><p className="text-sm font-semibold text-white">{isComplete ? "Your short is ready" : isFailed ? "The engine needs a hand" : "I’m building your short"}</p><p className="mt-1 text-xs leading-5 text-[#858b96]">{isComplete ? "The story, visuals, voice, and captions are assembled below." : isFailed ? currentLabel : "I’ll work through the story, visuals, voice, and timing for you."}</p></div></div><div className="generation-feed relative ml-1 pl-8"><span className="generation-feed-line absolute bottom-5 left-[11px] top-2 w-px" />{latestEvents.map((event, index) => <div key={event.id || `${event.stage}-${index}`} className={`generation-feed-row relative mb-4 flex gap-3 ${index === 0 && isProcessing ? "generation-feed-row-active" : ""}`}><span className={`generation-feed-emoji relative z-10 grid h-6 w-6 shrink-0 place-items-center rounded-lg text-sm ${index === 0 && isProcessing ? "generation-feed-emoji-active" : ""}`}>{event.level === "error" ? "🧯" : stageEmoji[event.stage || ""] || "✨"}</span><div className="min-w-0 pt-0.5"><p className="text-xs font-semibold text-[#e8e9ed]">{event.message}</p><p className="mt-1 text-[11px] leading-5 text-[#737a86]">{event.stage === "script" ? "Shaping the hook and narration beats." : event.stage === "terms" ? "Turning the idea into visual search signals." : event.stage === "audio" ? "Giving the narration a natural voice." : event.stage === "subtitle" ? "Matching captions to the spoken timing." : event.stage === "materials" ? "Finding footage that fits the story." : event.stage === "render" ? "Compositing everything into the final frame." : "The creative engine is getting ready."}</p></div><span className="ml-auto pt-1 text-[10px] text-[#666d79]">{event.timestamp ? new Date(event.timestamp * 1000).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : ""}</span></div>)}{isProcessing && <div className="generation-feed-row generation-feed-row-thinking relative flex gap-3"><span className="generation-feed-emoji generation-feed-emoji-active relative z-10 grid h-6 w-6 shrink-0 place-items-center rounded-lg text-sm">✨</span><div className="pt-0.5"><p className="text-xs font-semibold text-white">{currentLabel}</p><p className="mt-1 text-[11px] text-[#858b96]">Working on it<span className="generation-typing">...</span></p></div></div>}</div></div>
