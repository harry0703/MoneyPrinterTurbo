...
function GenerationDialog({ task, onMinimize, onClose }: { task: Task; onMinimize: () => void; onClose: () => void }) {
  ...
  return <aside className="generation-popup fixed bottom-5 right-5 z-40 flex max-h-[min(720px,calc(100vh-2rem))] w-[min(460px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-white/15 bg-[#15131b]/[.98] shadow-2xl shadow-black/50 backdrop-blur-xl" role="dialog" aria-label="Video generation progress">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">{isComplete && task.videos?.[0] && <div className="mb-5 overflow-hidden rounded-xl border border-white/10 bg-black"><div className="flex items-center justify-between border-b border-white/10 bg-white/[.03] px-3 py-2"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#858b96]">Generated preview</p><a className="text-[11px] font-semibold text-[#ff9b9f]" href={assetUrl(task.videos[0])} target="_blank" rel="noreferrer">Open file ↗</a></div><video className="max-h-72 w-full bg-black object-contain" controls playsInline src={assetUrl(task.videos[0])} /></div>}<div className="generation-scene relative mb-5 flex min-h-56 flex-col items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-[#111018] px-4 py-6 text-center"><div className="generation-grid absolute inset-0" /><span className="generation-doodle generation-doodle-one">{stageEmoji.script}</span><span className="generation-doodle generation-doodle-two">{stageEmoji.audio}</span><span className="generation-doodle generation-doodle-three">{stageEmoji.materials}</span><span className="generation-doodle generation-doodle-four">{stageEmoji.subtitle}</span><div className="generation-orb relative grid h-24 w-24 place-items-center rounded-full text-4xl"><span className="generation-orb-ring absolute inset-[-10px] rounded-full" /><span className="generation-orb-glow absolute inset-2 rounded-full" /><span className="relative z-10">{activeEmoji}</span></div><p className="relative z-10 mt-5 text-sm font-semibold text-white">{isComplete ? "The creative engine is done" : isFailed ? "The engine needs a hand" : "Tiny creative agents are at work"}</p><p className="relative z-10 mt-1 max-w-[270px] text-[11px] leading-5 text-[#858b96]">{isComplete ? "Your story, sound, and visuals are packed into the preview above." : isFailed ? currentLabel : "Mixing narration, visuals, and timing into one polished short."}</p></div><div className="mb-3 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#858b96]">Live activity</p><span className="text-[11px] text-[#666d79]">{activeStage} · {events.length} signals</span></div><div className="grid gap-2">{latestEvents.map((event, index) => <div key={event.id || `${event.stage}-${index}`} className={`generation-activity flex items-center gap-3 rounded-xl border px-3 py-2.5 ${index === 0 && isProcessing ? "generation-activity-active border-[#ff5b62]/35 bg-[#ff5b62]/[.08]" : "border-white/10 bg-white/[.02]"}`}><span className="text-lg">{event.level === "error" ? "🧯" : stageEmoji[event.stage || ""] || "✨"}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-[#e8e9ed]">{event.message}</p><p className="mt-0.5 text-[10px] text-[#666d79]">{event.stage || "engine"}{event.timestamp ? ` · ${new Date(event.timestamp * 1000).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : ""}</p></div>{index === 0 && isProcessing && <span className="generation-signal h-2 w-2 rounded-full bg-[#ff7a82]" />}</div>)}</div></div>
