...
export default function Home() {
  ...
  const saveSettings = async () => {
    ...
    try {
      await Promise.all([
        api("/settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ section: "app", values: { llm_provider: settingsDraft.llmProvider, openai_api_key: settingsDraft.llmApiKey || undefined, openai_base_url: settingsDraft.llmBaseUrl || undefined, openai_model_name: settingsDraft.llmModel || undefined, pexels_api_keys: settingsDraft.pexels || undefined, pixabay_api_keys: settingsDraft.pixabay || undefined, coverr_api_keys: settingsDraft.coverr || undefined, gemini_api_key: settingsDraft.geminiKey || undefined, minimax_api_key: settingsDraft.minimaxKey || undefined } }) }),
        api("/settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ section: "azure", values: { speech_key: settingsDraft.azureKey || undefined, speech_region: settingsDraft.azureRegion || undefined } }) }),
      ]);
...
function Panel({ title, eyebrow, accent, children }: { title: string; eyebrow: string; accent?: boolean; children: ReactNode }) {
  return <section className={`studio-panel flex min-h-0 flex-col rounded-2xl border ${accent ? "border-[#ff5b62]/40" : "border-white/10"} bg-[#15181d]/85 p-3.5 shadow-xl shadow-black/10`}><div className="mb-3 flex shrink-0 items-center justify-between border-b border-white/10 pb-2.5"><div className="flex items-center gap-2"><span className="grid h-5 w-5 place-items-center rounded-md bg-white/[.07] text-[9px] font-bold text-[#9899aa]">{eyebrow}</span><h2 className="text-sm font-bold tracking-[-.02em] text-white">{title}</h2></div><span className="h-1.5 w-1.5 rounded-full bg-[#ff5b62] opacity-70" /></div><div className="panel-scroll min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">{children}</div></section>;
}
...
function SettingsDrawer({ draft, setDraft, settings, onClose, onSave }: { draft: { llmProvider: string; llmApiKey: string; llmBaseUrl: string; llmModel: string; pexels: string; pixabay: string; coverr: string; azureKey: string; azureRegion: string; geminiKey: string; minimaxKey: string }; setDraft: (value: { llmProvider: string; llmApiKey: string; llmBaseUrl: string; llmModel: string; pexels: string; pixabay: string; coverr: string; azureKey: string; azureRegion: string; geminiKey: string; minimaxKey: string }) => void; settings: SettingsMap; onClose: () => void; onSave: () => Promise<void> }) {
  ...
  return <aside className="fixed inset-y-0 right-0 z-20 w-full max-w-xl overflow-y-auto border-l border-white/10 bg-[#111419] p-5 shadow-2xl"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#ff8f94]">Runtime configuration</p><h2 className="mt-1 text-xl font-bold">Settings</h2></div><button className="button" onClick={onClose}><X size={16} /></button></div><p className="mt-3 rounded-lg border border-amber-400/20 bg-amber-400/[.05] p-3 text-xs leading-5 text-amber-100">These values are saved by the Python engine and used by both the Next frontend and existing CLI workflows. Secret fields are write-only in the browser.</p><div className="mt-5 grid gap-4"><section className="grid gap-3"><h3 className="text-xs font-bold uppercase tracking-[.15em] text-[#9899aa]">Language model</h3><Field label="Provider"><input className="control" value={draft.llmProvider} onChange={(event) => update("llmProvider", event.target.value)} placeholder="openai, gemini, ollama..." /></Field><Field label={`API key${configured("app", `${draft.llmProvider}_api_key`) ? " · configured" : ""}`}><input className="control" type="password" value={draft.llmApiKey} onChange={(event) => update("llmApiKey", event.target.value)} /></Field><Field label="Base URL"><input className="control" value={draft.llmBaseUrl} onChange={(event) => update("llmBaseUrl", event.target.value)} placeholder="https://api.openai.com/v1" /></Field><Field label="Model name"><input className="control" value={draft.llmModel} onChange={(event) => update("llmModel", event.target.value)} placeholder="gpt-4o-mini" /></Field></section><section className="grid gap-3"><h3 className="text-xs font-bold uppercase tracking-[.15em] text-[#9899aa]">Material providers</h3><Field label="Pexels API key"><input className="control" type="password" value={draft.pexels} onChange={(event) => update("pexels", event.target.value)} /></Field><Field label="Pixabay API key"><input className="control" type="password" value={draft.pixabay} onChange={(event) => update("pixabay", event.target.value)} /></Field><Field label="Coverr API key"><input className="control" type="password" value={draft.coverr} onChange={(event) => update("coverr", event.target.value)} /></Field></section><section className="grid gap-3"><h3 className="text-xs font-bold uppercase tracking-[.15em] text-[#9899aa]">Voice providers</h3><div className="grid grid-cols-2 gap-3"><Field label="Azure speech key"><input className="control" type="password" value={draft.azureKey} onChange={(event) => update("azureKey", event.target.value)} /></Field><Field label="Azure region"><input className="control" value={draft.azureRegion} onChange={(event) => update("azureRegion", event.target.value)} /></Field></div><Field label="Gemini TTS key"><input className="control" type="password" value={draft.geminiKey} onChange={(event) => update("geminiKey", event.target.value)} /></Field><Field label="MiniMax key"><input className="control" type="password" value={draft.minimaxKey} onChange={(event) => update("minimaxKey", event.target.value)} /></Field></section></div><div className="mt-6 flex justify-end gap-2"><button className="button" onClick={onClose}>Cancel</button><button className="button button-primary" onClick={onSave}><Check size={14} /> Save settings</button></div></aside>;
}
