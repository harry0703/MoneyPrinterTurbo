...
Gemini embedding → pgvector HNSW search
   + tag-only fallback для ассетов без vector(768)
   ↓
...
Ассеты без embedding тоже попадают в пул, но только при содержательном
`photo_prefer_tags` или `photo_only_tags`: `only` имеет приоритет при выборе тегов
для fallback, иначе используются `prefer`. Для них semantic cosine равен 0;
`photo_only_tags` сам по себе не даёт tag-score — этот вклад создаёт только
совпадение с `photo_prefer_tags`. Fallback соблюдает `photo_exclude_tags`, уже
использованные id и удалённые файлы, а при объединении с векторной выдачей
дедуплицирует ассеты по id. Без `prefer`/`only` произвольные записи без вектора в
обычный retrieval не допускаются. Механика `photo_require` от embedding не
зависит и работает как прежде.
...
3. **Ассет без embedding не имеет semantic cosine.** Он доступен обычному
   retrieval только через содержательный `photo_prefer_tags`/`photo_only_tags` и
   ранжируется с cosine 0; без этих тегов остаются `photo_require` либо backfill.
   Свежескачанный scout-ассет вставляется напрямую только в текущем прогоне.
