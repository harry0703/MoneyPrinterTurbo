...
- [x] **Scene planner and alignment (first pass).** `app/services/scene_planner.py` splits the spoken script into exact sentence spans and aligns their boundaries to subtitle cues when the words match; otherwise it estimates timing and requires review. It runs after audio/subtitles and before material retrieval when `director_mode=true`, and persists `production-plan-v1.json`. Custom audio always requires review. The resulting video still uses the legacy renderer until the later Phase 1 tasks are complete.
