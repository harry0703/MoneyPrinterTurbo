...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=MoneyPrinterTurbo)** 是一个全模态 AI 推理平台，用单个 OpenAI 兼容 API 即可为 MoneyPrinterTurbo 的文案生成接入 DeepSeek、Qwen、GLM、Kimi、MiniMax 等模型，无需逐家对接。预算友好的 [coding plan](https://www.atlascloud.ai/console/coding-plan)。

在 WebUI 里把 `LLM Provider` 选成 `Atlas Cloud`，或在 `config.toml` 中配置：
...
`deepseek-ai/deepseek-v4-pro` 是推理模型，自定义调用时 `max_tokens` 建议给到 `>= 512`。
