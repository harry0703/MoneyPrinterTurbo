...
另外，你也可以将生成的视频推送到本地的 Instagram Graph API 发布端点。适用于你已经在同一台机器或同一网络中运行了自定义 Instagram 发布服务的情况。在 `config.toml` 的 `[app]` 下添加：

```toml
[app]
local_instagram_enabled = true
local_instagram_endpoint = "http://localhost:3000/upload"
local_instagram_description_template = "{subject}"
local_instagram_timeout = 600
```

`local_instagram_description_template` 支持 `{subject}` 和 `{caption}` 占位符。`{caption}` 默认取自生成的视频脚本（截取前 2100 个字符）。端点需接收 JSON 格式的 `filePath` 和 `description`，成功时返回 `{ "success": true, "postId": "..." }`。
