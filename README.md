...
English | [简体中文](README-zh.md) | [Releases](https://github.com/harry0703/MoneyPrinterTurbo/releases) | [Issues](https://github.com/amine9812/MoneyPrinterTurbo/issues)
...
- Fastest local start: clone the repo and run **one command** — `webui.bat` on Windows, `sh webui.sh` on macOS/Linux. Everything else installs automatically on first run
- If you do not want to install anything at all: generate videos with an AI Agent or use the Google Colab notebook
- If you want a fully isolated runtime: use Docker deployment
- Prefer manual control? See [Manual Deployment](#manual-deployment-) below

### One-Command Setup ⚡

Clone the repository, then run a single command. On first run it automatically installs [uv](https://docs.astral.sh/uv/), Python 3.11, and all dependencies, creates `config.toml` from the example file, and then launches the WebUI in your browser:

###### Windows (CMD or PowerShell)

```powershell
git clone https://github.com/amine9812/MoneyPrinterTurbo.git
cd MoneyPrinterTurbo
.\webui.bat
```

You can also just **double-click `webui.bat`** in File Explorer — no terminal needed.
...
If you prefer to set up without launching anything yet, run the dedicated installer instead:
...
Once the WebUI opens, configure your LLM provider and API keys in the on-screen basic settings — no config file editing required.
...
```text
Use this Skill: https://raw.githubusercontent.com/amine9812/MoneyPrinterTurbo/main/docs/skill/SKILL.md
...
### Windows Prebuilt Package (optional)

Prefer a downloadable bundle with everything included? Get the one-click package maintained by the upstream project (extract to a path without **non-ASCII characters**, **special characters**, or **spaces**):
...
`webui.bat` prefers the project `.venv` or bundled Python from the portable package. If no project Python is found but `uv` is installed, it automatically falls back to `uv run streamlit`. If neither exists, it runs `install.bat` automatically to perform the first-time setup, then continues.
...
The script automatically uses the project virtual environment or `uv` and selects an available local port. If no Python environment and no `uv` exist, it runs `install.sh` automatically to perform the first-time setup, then continues. To allow access from other devices on your LAN, run:
...
- You can submit an [issue](https://github.com/amine9812/MoneyPrinterTurbo/issues) or a [pull request](https://github.com/amine9812/MoneyPrinterTurbo/pulls).
