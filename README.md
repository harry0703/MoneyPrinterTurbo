...
- [x] 支持 **客服代表培训模式**，可配置培训主题、场景类型、客户画像、政策上下文，并生成适合新人培训的结构化讲解脚本
...
MoneyPrinterTurbo 可以作为客服代表微课视频生成器使用。在 WebUI 中选择 `CSR Training` 内容模式后，可以选择或自定义：
...
CSR 脚本生成器会生成简短的教练式旁白：先说明客户场景，再展示应避免的回应、推荐的更好回应、原因解释和关键 takeaway。生成的脚本会继续复用现有的 TTS、字幕、背景音乐以及 stock/local 视频素材合成流程。

如果需要更长的培训内容，可以选择 `CSR Long Training`。长视频会按章节生成：

1. 先根据目标时长生成培训大纲。
2. 每个章节分别生成脚本、语音、字幕、stock/local 素材和独立 MP4。
3. 最后把所有章节视频拼接成一个完整培训视频。

这种分章节渲染方式更适合 15-20 分钟的 CSR 培训课，可以避免一次性在 MoviePy 中加载超长合成任务。
...
建议把真实客服政策、升级规则和知识库内容粘贴到 policy context 中。内置政策只是通用培训示例，不应视为公司专属的法律、财务、医疗或合规建议。
