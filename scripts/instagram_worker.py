...
def _recent_clips(client, amount: int):
    """
    取最近若干条作品，优先走网页版的 GraphQL 接口。

    上一版直接用 ``user_clips``，走的是手机应用那套私有接口。四个账号里有三个
    在那次调用之后随即失效，虽然无法确证是它造成的，但网页接口更接近浏览器
    自己会发的请求，风险面更小。取不到时才退回私有接口，并把实际走的路径带
    回结果里——否则悄悄降级就等于没有做这个选择。
    """
    try:
        media = client.user_medias_gql(client.user_id, amount=amount)
        if media:
            return media, "gql"
    except Exception as exc:
        log(f"gql media listing unavailable, falling back: {type(exc).__name__}")

    return client.user_clips(client.user_id, amount=amount), "v1"
...
def _stats(client, request: dict) -> dict:
    ...
    clips, source = _recent_clips(client, amount)
