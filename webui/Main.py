...
CREDENTIAL_WIDGET_STATE_ALIASES = {
    ...
    ("app", "pexels_api_keys"): ("selected_pexels_api_keys_input",),
    ("app", "pixabay_api_keys"): ("selected_pixabay_api_keys_input",),
    ("app", "coverr_api_keys"): ("selected_coverr_api_keys_input",),
}
...
def _apply_restored_params(params):
    ...
    if video_source == "pexels_pixabay":
        ...
        # The source selector shows the generic stock option while the provider
        # selector carries the actual Pexels -> Pixabay order.
        video_source = "pexels"
    ...
    if video_source in stock_sources:
        ...
        _set_stable_widget_value("material_provider_mode_select", mode)
        _set_stable_widget_value("material_providers_select", providers)
    _set_stable_widget_value(
        "video_source_select",
        "stock" if video_source in stock_sources else video_source,
    )
    _set_stable_widget_value(
...
def _material_provider_has_key(provider):
    """Return whether a selected stock provider has at least one usable key."""
    configured_keys = config.app.get(f"{provider}_api_keys")
    if isinstance(configured_keys, str):
        return bool(configured_keys.strip())
    return bool(
        configured_keys and any(str(key).strip() for key in configured_keys)
    )


def _render_selected_material_provider_key_cards(providers):
    """Show an inline setup card for every selected stock provider missing a key."""
    provider_names = {
        "pexels": tr("Pexels"),
        "pixabay": tr("Pixabay"),
        "coverr": tr("Coverr"),
    ...
    api_key_labels = {
        ...
        "coverr": "Coverr API Key",
    }
    for provider in providers:
        if provider not in provider_names or _material_provider_has_key(provider):
            continue
        with st.container(border=True):
            st.caption(f"{provider_names[provider]} · {tr('API Key Missing')}")
            config_key = f"{provider}_api_keys"
            api_key = st.text_input(
                tr(api_key_labels[provider]),
                value=_get_material_api_keys(config_key),
                type="password",
                key=f"selected_{provider}_api_keys_input",
            )
            _save_material_api_keys(config_key, api_key)
    ...
    with panel:
        with st.container(border=True):
            ...
            saved_stock_providers = [
                ...
                if provider in stock_sources
            ]
            # ``pexels_pixabay`` was a legacy shorthand, not a separate backend.
            # Restore it as stock fan-out with explicit Pexels -> Pixabay priority.
            legacy_pexels_pixabay = saved_video_source_name == "pexels_pixabay"
            selected_source_type = stable_selectbox(
                tr("Video Source"),
                options=[value for _, value in video_source_types],
                default_value=(
                    "stock"
                    if legacy_pexels_pixabay
                    or saved_video_source_name in stock_sources
                    else saved_video_source_name
                ),
                key="video_source_select",
                format_func=lambda value: dict(
                    (v, label) for label, v in video_source_types
                )[value],
            )

            if selected_source_type == "stock":
                strategy_modes = ["locked", "fallback", "fan_out"]
                strategy_labels = {
                ...
                if saved_mode not in strategy_modes:
                    saved_mode = "locked"
                params.material_provider_mode = stable_selectbox(
                    tr("Material Provider Mode"),
                    options=strategy_modes,
                    default_value=saved_mode,
                ...
                configured_providers = (
                    ...
                    or (
                        ...
                        if saved_video_source_name in stock_sources
                        else ["pexels"]
                    )
                )
                ...
                provider_widget_key = localized_widget_key(
                    "material_providers_select"
                )
                current_providers = st.session_state.get(provider_widget_key)
                if isinstance(current_providers, (list, tuple)):
                    normalized_current = [
                        provider
                        for provider in current_providers
                        if provider in stock_sources
                    ]
                    if current_providers and not normalized_current:
                        normalized_current = configured_providers
                else:
                    normalized_current = configured_providers
                if params.material_provider_mode == "locked":
                    normalized_current = normalized_current[:1]
                st.session_state[provider_widget_key] = normalized_current

                params.material_providers = st.multiselect(
                    tr("Material Providers"),
                    options=list(provider_names),
                    key=provider_widget_key,
                    help=tr("Material Providers Help"),
                    max_selections=(
                        1 if params.material_provider_mode == "locked" else None
                    ),
                    format_func=lambda provider: (
                        f"{provider_names[provider]} ({tr('API Key Missing')})"
                        if not _material_provider_has_key(provider)
                ...
                if not params.material_providers:
                    ...
                    # ``video_source`` remains an API/task compatibility field.
                    # It mirrors the first explicit provider but never injects one
                    # into the user's provider selection.
                    params.video_source = params.material_providers[0]
                    _set_runtime_config(
                        "app", "video_source", params.video_source
                    )
                _set_runtime_config(
                    "app",
                    "material_provider_mode",
                    params.material_provider_mode,
                )
                _set_runtime_config(
                    "app", "material_providers", params.material_providers
                )
                _render_selected_material_provider_key_cards(
                    params.material_providers
                )
                if params.material_provider_mode == "fan_out":
                    st.warning(tr("Fan-out Quota Warning"))
            else:
                params.video_source = selected_source_type
