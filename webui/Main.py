...
_RUNTIME_CONFIG_SECTIONS = {
    ...
    "volcengine_tts": config.volcengine_tts,
    "ui": config.ui,
}
...
def _infer_tts_server_from_voice(voice_name):
    ...
    if voice.is_chatterbox_voice(voice_name):
        return "chatterbox"
    if voice.is_volcengine_voice(voice_name):
...
def _get_voice_preview_provider_signature(tts_server: str) -> dict:
    ...
    if tts_server == "volcengine-tts":
        return {
            "model_id": config.volcengine_tts.get("model_id", ""),
            "credential": _credential_signature(
                config.volcengine_tts.get("api_key", "")
    ...
    with panel:
        with st.container(border=True):
            ...
            def _friendly(v):
                ...
                if voice.is_volcengine_voice(v):
                    name = v.split(":", 1)[1] if ":" in v else v
                    return name.replace("-Female", "").replace("-Male", "")
            ...
            # VolcEngine Ark TTS settings (豆包语音合成). Reuses the volcengine
            # LLM API key and Base URL by default, with optional dedicated overrides.
            if tts_mode_enabled and (
                selected_tts_server == "volcengine-tts"
                or (voice_name and voice.is_volcengine_voice(voice_name))
            ):
                saved_volcengine_tts_key = config.volcengine_tts.get("api_key", "")
                volcengine_tts_api_key = st.text_input(
                    tr("VolcEngine TTS API Key"),
                    value=saved_volcengine_tts_key,
                    type="password",
                    key="volcengine_tts_api_key_input",
                    placeholder=tr("Uses VolcEngine LLM Key if empty"),
                )
                _set_runtime_config(
                    "volcengine_tts", "api_key", volcengine_tts_api_key
                )
