...
def account_labels() -> list[str]:
    """只列出允许读数据的账号，见 instagram.STATS_ACCOUNTS 上的说明。"""
    from app.services import instagram

    try:
        return list(instagram.stats_accounts())
    except Exception:
        return []


def withheld() -> list[str]:
    """配置了但暂时不读的账号，写在页面上，免得看着像漏了。"""
    from app.services import instagram

    try:
        allowed = set(instagram.stats_accounts())
        return [a.label for a in instagram.list_accounts() if a.label not in allowed]
    except Exception:
        return []
...
if not labels:
    st.warning("No account is enabled for reading stats.")
    st.stop()

held = withheld()
if held:
    st.info(
        f"Only {', '.join(labels)} is read here. Three sessions expired right "
        f"after the first run of this page, so {', '.join(held)} stay out until "
        "the throwaway account has gone a few days unaffected."
    )
