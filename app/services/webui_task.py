...
def submit_generation(
    ...
    # 用户会话凭证只能随当前进程内任务传递，不能进入 VideoParams、任务状态、
    # 日志或落盘历史。这里只复制外层容器，内部不可变设置和批次对象保持原值。
    runtime_context_snapshot = dict(runtime_context) if runtime_context else None
    sm.state.update_task(
        task_id,
    ...
    try:
        _task_manager.add_task(
            ...
            runtime_context=runtime_context_snapshot,
        )
    except Exception as exc:
