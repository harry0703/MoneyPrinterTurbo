...
設定値が空の場合は `ARK_API_KEY` と `VOLCENGINE_ARK_API_KEY` 環境変数も使用できます。動画ソースで **Volcano Engine Seedance** を選び、生成前に有料タスクを明示的に確認してください。CLI では `--confirm-seedance-charge` も必要です。
