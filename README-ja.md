...
後方互換性のため、すべての `/api/v1` エンドポイントはデフォルトでは認証なしです。
保護するには `config.toml` の `[app]` に `api_key` を設定してください。その後、
クライアントは同じ値を `x-api-key` ヘッダーで送信する必要があります。
ヘルスチェックの `/ping` は引き続き公開されます。
