...
専用設定が空の場合は、`OFOX_API_KEY` 環境変数を使用します。動画ソースで **OFox AI Video** を選び、生成前に有料タスクを明示的に確認してください。CLI では `--confirm-ofox-charge` も必要です。上流ベンダーは既定で `"byteplus"`（国際ベンダー。コンテンツポリシーが一貫し、ルーティングが予測可能）に固定されます。`ofox_provider` を `"volcengine"`（中国本土ベンダー）に変更するか、空文字列 `""` を設定するとゲートウェイが重みに基づいて分散します。モデルは OFox カタログ（`GET /v1/models`）の任意のテキスト動画モデル（例：`alibaba/wan-2.7`）に変更できます。モデルごとに対応する動画長と解像度が異なり、範囲外の値はタスク作成前に API が明確なエラーで拒否します。
